...
def unpack_matrix(packed_vals: np.ndarray, packed_indices: np.ndarray, shape=(16, 16)) -> np.ndarray:
    unpacked = np.zeros(shape, dtype=packed_vals.dtype)
    rows, cols = packed_vals.shape

    for i in range(rows):
        for j in range(cols):
            idx = packed_indices[i, j]
            if idx != -1:
                unpacked[i, idx] = packed_vals[i, j]
    
    return unpacked

def pad_to_tile_size(matrix, tile_size, pad_value=0):
    rows, cols = matrix.shape
    pad_rows = (tile_size - rows % tile_size) % tile_size
    pad_cols = (tile_size - cols % tile_size) % tile_size
    return np.pad(matrix, ((0, pad_rows), (0, pad_cols)), mode='constant', constant_values=pad_value)

def mult(packed: np.ndarray, indices: List[List[int]], x: np.ndarray) -> np.ndarray:
    y = np.zeros(len(indices), dtype=packed.dtype)
    for i, idx in enumerate(indices):
        # dot the i‑th row of packed with the corresponding entries of x
        y[i] = packed[i].dot(x[idx])
    return y
